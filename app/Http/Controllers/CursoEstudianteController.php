		...
		if ($curso) {
			...
			if ($estudiante) {
				...
				if ($estudiantes->find($estudiante_id)) {
					return $this->crearRespuestaError("El estudiante $estudiante_id ya existe en este curso",409);
				}
			...
			return $this->crearRespuestaError("No se puede encontrar un estudiante con el id $estudiante_id",404);
		}
		return $this->crearRespuestaError("No se puede encontrar un curso con el id $curso_id",404);
	}
